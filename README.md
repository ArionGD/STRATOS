...
separated per user. Locally it uses an embedded Postgres (PGlite) in `server/.data/`; no install needed.
```bash
...
`render.yaml` defines a free web service plus a free Postgres database.
...
| Env var | Purpose |
| --- | --- |
| `DATABASE_URL` | Postgres connection (wired from the Blueprint database) |
| `JWT_SECRET` | Signs login sessions (auto-generated by Render) |
| `SEED_DEMO_USER` | `true` creates the `test` / `pass` account on boot |
| `NODE_OPTIONS` | `--max-old-space-size=320` keeps the build (~395 MB peak) inside the 512 MB free instance |
| `PG_POOL_MAX` | Optional cap on Postgres connections (default 10) |
| `VITE_ENABLE_AI` | `true` shows METIS AI in the web build (hidden by default; always on in the desktop app) |

Free-tier notes: the service sleeps after 15 min idle (first request takes ~1 min to wake),
and the free Postgres database expires after 30 days.
