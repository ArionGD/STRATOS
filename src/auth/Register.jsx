  ...
  const handleSignUp = async (e) => {
    ...
    if (isTauri) {
      ...
      try {
        ...
        setStatus({ type: 'success', message: 'User created in Browser DB! Forwarding to Login...' });
        setTimeout(() => navigate('/login'), 2000);
      } catch (err) {
        setStatus({ type: 'error', message: 'Registration failed: ' + err });
      }
    }
  }
            ...
            <div className="grid grid-cols-2 gap-4">
               <div className="space-y-1.5">
                  ...
                  <div className="relative group">
                    ...
                    <input type="text" required value={formData.firstName} onChange={(e) => setFormData({...formData, firstName: e.target.value})} placeholder="Aditya" className="w-full bg-slate-50 border border-slate-100 rounded-xl px-12 py-3 text-sm text-slate-900 outline-none focus:border-blue-500 focus:bg-white transition-all shadow-sm" />
                  </div>
               </div>
               <div className="space-y-1.5">
                  ...
                  <div className="relative group">
                    <User size={16} className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-blue-600 transition-colors" />
                    <input type="text" required value={formData.lastName} onChange={(e) => setFormData({...formData, lastName: e.target.value})} placeholder="Pandey" className="w-full bg-slate-50 border border-slate-100 rounded-xl px-12 py-3 text-sm text-slate-900 outline-none focus:border-blue-500 focus:bg-white transition-all shadow-sm" />
                  </div>
               </div>
            </div>
