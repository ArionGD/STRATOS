...
import { Mail, Lock, EyeOff, Layout, ChevronLeft, ChevronRight, Brain, Zap, Shield, User, Heart, Sparkles, Cpu, ArrowLeft } from 'lucide-react'
...
const Login = () => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      const user = await invoke('login_user', { email: formData.email, password: formData.password })
      setStatus({ type: 'success', message: `Welcome back, ${user.name}!` })
          ...
          <form className="space-y-4" onSubmit={handleLogin}>
            <div className="space-y-1.5">
              ...
              <div className="relative group">
                ...
                <input type="email" required value={formData.email} onChange={(e) => setFormData({...formData, email: e.target.value})} placeholder="eg. aditya@stratos.com" className="w-full bg-slate-50 border border-slate-100 rounded-xl px-12 py-3 text-sm text-slate-900 outline-none focus:border-blue-500 focus:bg-white transition-all shadow-sm" />
              </div>
            </div>
