...
const Login = () => {
  ...
  const handleLogin = async (e) => {
    ...
    if (isTauri) {
      try {
        const user = await invoke('login_user', { username: formData.username, password: formData.password })
        setUser(user)
          ...
          <form className="space-y-4" onSubmit={handleLogin}>
            <div className="space-y-1.5">
              ...
              <div className="relative group">
                ...
                <input type="text" required value={formData.username} onChange={(e) => setFormData({...formData, username: e.target.value})} placeholder="eg. aditya" className="w-full bg-slate-50 border border-slate-100 rounded-xl px-12 py-3 text-sm text-slate-900 outline-none focus:border-blue-500 focus:bg-white transition-all shadow-sm" />
              </div>
            </div>
            ...
            <div className="space-y-1.5">
              ...
              <div className="relative group">
                ...
                <button type="button" className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-600"><EyeOff size={16} /></button>
              </div>
            </div>
            ...
            <div className="flex items-center justify-between px-1 pt-1 gap-2">
              ...
              <a href="/register" className="py-2 md:py-0 text-xs font-bold text-slate-400 hover:text-blue-600 transition-colors">Register</a>
