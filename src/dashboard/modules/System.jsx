    ...
    >
      <header className={`h-auto md:h-24 border-b border-white/5 flex flex-col md:flex-row items-stretch md:items-center justify-between gap-4 md:gap-0 px-4 py-5 md:py-0 md:px-10 shrink-0 ${line}`}>
        <div className="flex items-center gap-3 md:gap-4 max-md:min-w-0">
          <div className={`w-11 h-11 md:w-12 md:h-12 max-md:shrink-0 rounded-2xl bg-white/5 border border-white/10 flex items-center justify-center text-slate-400 ${tile}`}>
            <Settings size={24} />
          </div>
          <div className="max-md:min-w-0">
            <h1 className="text-[26px] max-md:leading-tight md:text-2xl font-black uppercase tracking-tighter">System <span className="text-slate-500">Core</span></h1>
        ...
        <div className="grid grid-cols-2 md:flex md:items-center gap-3 md:gap-4">
          <button className={`flex items-center justify-center md:justify-start gap-2 px-3 md:px-4 py-2 max-md:min-h-[44px] bg-white/5 border border-white/10 rounded-xl text-xs font-bold hover:bg-white/10 transition-all ${tile}`}>
            <RefreshCw size={14} /> Restart Kernel
          </button>
          <button className={`px-3 md:px-6 py-2.5 max-md:min-h-[44px] max-md:text-sm bg-white text-[#0F172A] rounded-xl font-bold flex items-center justify-center md:justify-start gap-2 shadow-xl hover:bg-slate-200 transition-all ${lt ? 'max-md:bg-[#0F172A] max-md:text-white' : ''}`}>
            <Terminal size={18} /> Open Console
          </button>
        </div>
      </header>

      <div className="flex-none md:flex-1 overflow-visible md:overflow-y-auto no-scrollbar p-4 md:p-10 space-y-4 md:space-y-10">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-6">
          {stats.map((s, idx) => (
            <div key={idx} className={`p-4 md:p-8 rounded-2xl md:rounded-[2.5rem] max-md:min-w-0 border border-white/5 bg-white/2 backdrop-blur-xl group hover:border-white/20 transition-all ${card}`}>
              <div className={`w-10 h-10 rounded-xl bg-white/5 flex items-center justify-center mb-4 md:mb-6 ${s.iconColor} ${lt ? 'max-md:bg-slate-100' : ''}`}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
          <section className={`p-5 md:p-10 rounded-2xl md:rounded-[3.5rem] border border-white/5 bg-white/2 backdrop-blur-xl space-y-5 md:space-y-8 ${card}`}>
            <div className="flex items-center gap-3 md:gap-4">
              <div className="w-11 h-11 md:w-12 md:h-12 max-md:shrink-0 rounded-2xl bg-blue-600/10 flex items-center justify-center text-blue-500">
                <Shield size={24} />
              </div>
              <div>
            ...
            <div className="space-y-1 md:space-y-4">
              {[
                ...
                <div key={idx} className={`flex items-center justify-between gap-3 md:gap-0 max-md:min-h-[48px] py-3 md:py-4 border-b border-white/5 ${line}`}>
          ...
          <section className={`p-5 md:p-10 rounded-2xl md:rounded-[3.5rem] border border-white/5 bg-white/2 backdrop-blur-xl space-y-5 md:space-y-8 ${card}`}>
            <div className="flex items-center gap-3 md:gap-4">
              <div className="w-11 h-11 md:w-12 md:h-12 max-md:shrink-0 rounded-2xl bg-amber-500/10 flex items-center justify-center text-amber-500">
                <Database size={24} />
              </div>
              <div>
            ...
            <div className="space-y-5 md:space-y-6">
              ...
              <div className="grid grid-cols-2 gap-3 md:gap-4">
                <button className={`p-4 md:p-6 rounded-2xl bg-white/5 border border-white/10 text-center hover:bg-white/10 transition-all ${tile}`}>
                ...
                <button className="p-4 md:p-6 rounded-2xl bg-red-500/5 border border-red-500/10 text-center hover:bg-red-500/10 transition-all group">
        ...
        <footer className="text-center py-6 md:py-10 opacity-30">
