...
const InfoPage = ({ theme, onClose }) => {
  ...
  return (
    ...
    >
      <header className={`py-3 md:py-0 md:h-24 flex items-center justify-between gap-3 px-4 md:px-10 shrink-0 border-b backdrop-blur-xl ${isDark ? 'border-white/5 bg-white/5' : 'border-slate-200 bg-white/80'}`}>
        <div className="flex items-center gap-3 md:gap-4 min-w-0">
          <div className="w-10 h-10 md:w-12 md:h-12 shrink-0 rounded-xl md:rounded-2xl bg-blue-500/20 flex items-center justify-center text-blue-500">
            <Info size={24} className="w-5 h-5 md:w-6 md:h-6" />
          </div>
          <div className="min-w-0">
            <h1 className="text-lg leading-tight md:text-2xl font-black uppercase tracking-tighter">System Intelligence</h1>
            <p className="text-[9px] max-md:leading-snug max-md:mt-0.5 md:text-[10px] text-slate-500 font-bold uppercase tracking-wider md:tracking-widest">Core Architecture & Protocol Information</p>
          </div>
        </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 py-6 md:p-10 space-y-8 md:space-y-12 no-scrollbar">
        {/* Core Identity */}
        <section className="max-w-4xl mx-auto flex flex-col md:flex-row items-center gap-6 md:gap-12 py-2 md:py-10">
          <div className="w-28 h-28 md:w-48 md:h-48 bg-gradient-to-br from-blue-600 to-indigo-700 rounded-[2rem] md:rounded-[3rem] shadow-2xl flex items-center justify-center text-white font-black text-5xl md:text-7xl shrink-0 rotate-3 hover:rotate-0 transition-transform duration-500">
            S
          </div>
          <div className="space-y-4 md:space-y-6 max-md:text-center">
            <h2 className="text-[30px] md:text-5xl font-black tracking-tighter leading-none md:leading-none">Stratos <span className="text-blue-500">v1.2.2</span></h2>
            <p className={`text-[15px] md:text-lg font-medium leading-relaxed md:leading-relaxed ${isDark ? 'text-slate-400' : 'text-slate-600'}`}>
        ...
        {/* Tech Stack Grid */}
        <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3 md:gap-6 max-w-6xl mx-auto">
          {[
            ...
            <div key={idx} className={`p-5 md:p-8 rounded-2xl md:rounded-[2.5rem] border transition-all group ${isDark ? 'border-white/5 bg-white/5 hover:border-blue-500/30' : 'border-slate-200 bg-white hover:border-blue-500/30 shadow-sm hover:shadow-md'}`}>
              <item.icon className={`${item.color} mb-3 md:mb-6 w-7 h-7 md:w-8 md:h-8 group-hover:scale-110 transition-transform`} size={32} />
              <h3 className="text-[17px] md:text-xl font-bold mb-1.5 md:mb-3">{item.title}</h3>
        ...
        {/* System Stats */}
        <section className={`max-w-4xl mx-auto p-5 md:p-10 rounded-2xl md:rounded-[3rem] border relative overflow-hidden ${isDark ? 'bg-gradient-to-br from-blue-600/10 to-transparent border-white/5' : 'bg-blue-50 border-blue-100'}`}>
          <div className="absolute top-0 right-0 w-64 h-64 bg-blue-500/10 blur-[100px] rounded-full -mr-32 max-md:mr-0 -mt-32"></div>
          <div className="relative z-10 grid grid-cols-2 md:grid-cols-4 gap-5 md:gap-10">
            <div>
              <div className={`text-2xl md:text-3xl font-black mb-1 ${isDark ? 'text-white' : 'text-blue-600'}`}>0.1s</div>
              <div className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Latency</div>
            </div>
            <div>
              <div className={`text-2xl md:text-3xl font-black mb-1 ${isDark ? 'text-white' : 'text-blue-600'}`}>128-bit</div>
              <div className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Encryption</div>
            </div>
            <div>
              <div className={`text-2xl md:text-3xl font-black mb-1 ${isDark ? 'text-white' : 'text-blue-600'}`}>Infinite</div>
              <div className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Nodes</div>
            </div>
            <div>
              <div className={`text-2xl md:text-3xl font-black mb-1 ${isDark ? 'text-white' : 'text-blue-600'}`}>v1.2.2</div>
        ...
        {/* Footer Links */}
        <section className={`flex flex-col items-center gap-4 md:gap-6 py-6 md:py-10 border-t ${isDark ? 'border-white/5' : 'border-slate-200'}`}>
