    ...
    >
      ...
      <header className="py-3 md:py-0 md:h-24 flex items-center justify-between gap-3 px-4 md:px-10 shrink-0 border-b border-white/5 bg-white/2 backdrop-blur-xl">
        <div className="flex items-center gap-3 md:gap-4 min-w-0">
          <div className="w-10 h-10 md:w-12 md:h-12 shrink-0 rounded-xl md:rounded-2xl bg-amber-500/20 flex items-center justify-center text-amber-500 shadow-lg shadow-amber-500/20">
            <Compass size={24} className="w-5 h-5 md:w-6 md:h-6" />
          </div>
          <div className="min-w-0">
            ...
            <p className="text-[9px] max-md:leading-snug max-md:mt-0.5 md:text-[10px] text-slate-500 font-bold uppercase tracking-wider md:tracking-widest">Mastering the Stratos Operating System</p>
          </div>
        </div>
      ...
      <div className="flex-1 overflow-y-auto no-scrollbar p-4 py-8 md:p-10 md:py-20">
        <div className="max-w-6xl mx-auto space-y-10 md:space-y-24">
          
          {/* Hero Section */}
          <section className="text-center space-y-4 md:space-y-6">
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-blue-500/10 border border-blue-500/20 text-blue-500 text-[10px] font-black uppercase tracking-widest mb-1 md:mb-4">
              <PlayCircle size={14} /> Technical Onboarding v2.1
            </div>
            <h2 className="text-[28px] md:text-6xl font-black tracking-tighter leading-tight md:leading-tight max-w-4xl mx-auto">
              Welcome to the future of <span className="text-blue-500">knowledge orchestration</span>.
            </h2>
            <p className="text-[15px] md:text-xl text-slate-500 font-medium max-w-2xl mx-auto leading-relaxed md:leading-relaxed">
          ...
          <section className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-8">
            {steps.map((step, idx) => (
              <motion.div 
                ...
                className={`p-5 md:p-10 rounded-2xl md:rounded-[3rem] border border-white/5 bg-white/2 backdrop-blur-xl group hover:border-white/10 transition-all flex gap-4 md:gap-8 items-start ${lc}`}
              >
                <div className={`w-12 h-12 md:w-20 md:h-20 rounded-xl md:rounded-[2rem] ${step.color} flex items-center justify-center text-white shrink-0 shadow-2xl group-hover:scale-110 transition-transform`}>
                  <step.icon size={36} className="w-6 h-6 md:w-9 md:h-9" />
                </div>
                <div className="space-y-1.5 md:space-y-3 min-w-0">
                  <h3 className="text-[17px] leading-snug md:text-2xl md:leading-8 font-black tracking-tight">{step.title}</h3>
                  <p className="max-md:text-sm text-slate-500 font-medium leading-relaxed">{step.desc}</p>
                  <button className="flex items-center gap-2 max-md:min-h-[40px] text-xs font-black text-blue-500 group-hover:gap-3 transition-all">
          ...
          {/* Interaction Guide */}
          <section className="p-5 md:p-16 rounded-2xl md:rounded-[4rem] bg-gradient-to-br from-blue-600/10 via-transparent to-transparent border border-blue-500/10 relative overflow-hidden">
             ...
             <div className="max-w-2xl space-y-6 md:space-y-10 relative">
               <div className="space-y-3 md:space-y-4">
                 ...
                 <h3 className="text-2xl leading-tight md:text-4xl md:leading-10 font-black tracking-tighter">The Power of the <span className="text-blue-500">Command Bar</span>.</h3>
                 <p className="text-[15px] md:text-lg text-slate-400 font-medium leading-relaxed md:leading-relaxed">
               ...
               <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-6">
                 <div className={`p-6 rounded-2xl bg-white/5 border border-white/5 ${lc}`}>
          ...
          {/* Footer Guide */}
          <section className="text-center py-6 md:py-20 space-y-5 md:space-y-8">
            <h3 className="text-lg md:text-2xl font-black tracking-tight">Need deeper assistance?</h3>
            <div className="flex flex-col md:flex-row gap-3 md:gap-4 justify-center">
              <button className="w-full md:w-auto px-6 py-3.5 md:px-10 md:py-4 bg-blue-600 rounded-2xl font-bold shadow-xl shadow-blue-600/20 hover:scale-105 transition-all">Visit Support Nexus</button>
              <button className={`w-full md:w-auto px-6 py-3.5 md:px-10 md:py-4 bg-white/5 border border-white/10 rounded-2xl font-bold hover:bg-white/10 transition-all ${lc}`}>Watch Technical Demo</button>
            </div>
            <p className="text-[10px] text-slate-600 font-black uppercase tracking-[0.25em] md:tracking-[0.4em] pt-4 md:pt-10">Stratos Guide • Revision 2.1.0 • May 2026</p>
          </section>
