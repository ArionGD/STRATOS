...
const WhatsNew = ({ theme, onClose }) => {
  ...
  return (
    ...
    >
      ...
      <header className="py-2 md:py-0 md:h-24 flex items-center justify-between gap-3 px-4 md:px-10 shrink-0 border-b border-white/5 bg-white/2 backdrop-blur-xl">
        <div className="flex items-center gap-3 md:gap-4 min-w-0">
          <div className="w-9 h-9 md:w-12 md:h-12 shrink-0 rounded-xl md:rounded-2xl bg-amber-500/20 flex items-center justify-center text-amber-500">
            <Sparkles size={24} className="w-[18px] h-[18px] md:w-6 md:h-6" />
          </div>
          <div className="min-w-0">
            <h1 className="text-xl leading-tight md:text-2xl font-black uppercase tracking-tighter">System Evolution</h1>
            <p className="max-md:hidden text-[9px] max-md:leading-snug max-md:mt-0.5 md:text-[10px] text-slate-500 font-bold uppercase tracking-wider md:tracking-widest">Version History & Feature Logs</p>
          </div>
        </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 pt-4 pb-6 md:p-10 max-w-4xl mx-auto w-full space-y-6 md:space-y-12 no-scrollbar">
        <div className="relative">
          {/* Vertical Line */}
          <div className={`absolute left-[19px] md:left-[31px] top-4 bottom-4 w-px bg-white/10 ${theme === 'dark' ? '' : 'max-md:bg-slate-200'}`}></div>
          ...
          <div className="space-y-6 md:space-y-16">
            {updates.map((upd, idx) => (
              <motion.div 
                ...
                className="relative pl-[52px] md:pl-20"
              >
                {/* Timeline Dot */}
                <div className={`absolute left-0 top-0 w-10 h-10 md:w-16 md:h-16 rounded-xl md:rounded-3xl ${upd.color} flex items-center justify-center text-white shadow-2xl z-10`}>
                  <upd.icon size={28} className="w-5 h-5 md:w-7 md:h-7" />
                </div>

                <div className="space-y-1.5 md:space-y-4">
                  <div className="flex items-center gap-3 max-md:min-h-[40px] max-md:-mb-0.5">
                  ...
                  <h2 className="text-lg leading-tight md:text-3xl md:leading-9 font-black tracking-tight">{upd.title}</h2>
                  <p className="text-slate-400 font-medium leading-relaxed md:leading-relaxed text-sm md:text-lg max-w-2xl">{upd.desc}</p>
                  <button className="flex items-center gap-2 max-md:min-h-[40px] max-md:-my-1 text-xs font-black text-blue-500 hover:text-blue-400 transition-all group">
        ...
        <section className={`mt-6 md:mt-20 p-5 md:p-10 rounded-2xl md:rounded-[3rem] border border-white/10 text-center space-y-3 md:space-y-6 ${theme === 'dark' ? 'bg-white/5' : 'bg-slate-50 max-md:bg-white max-md:border-slate-200 max-md:shadow-sm'}`}>
          <div className="w-12 h-12 md:w-16 md:h-16 bg-blue-600 rounded-2xl flex items-center justify-center text-white mx-auto shadow-xl">
            <Zap size={32} className="w-6 h-6 md:w-8 md:h-8" />
          </div>
          <h3 className="text-lg md:text-2xl md:leading-8 font-black tracking-tight">Stay ahead of the curve.</h3>
