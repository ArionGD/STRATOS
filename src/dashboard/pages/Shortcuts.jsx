...
const Shortcuts = ({ theme, onClose }) => {
  ...
  return (
    ...
    >
      <header className="py-3 md:py-0 md:h-24 flex items-center justify-between gap-3 px-4 md:px-10 shrink-0 border-b border-white/5">
        <div className="flex items-center gap-3 md:gap-4 min-w-0">
          <div className="w-10 h-10 md:w-12 md:h-12 shrink-0 rounded-xl md:rounded-2xl bg-indigo-500/20 flex items-center justify-center text-indigo-500">
            <Keyboard size={24} className="w-5 h-5 md:w-6 md:h-6" />
          </div>
          <div className="min-w-0">
            <h1 className="text-lg leading-tight md:text-2xl font-black uppercase tracking-tighter">Command Protocols</h1>
            <p className="text-[9px] max-md:leading-snug max-md:mt-0.5 md:text-[10px] text-slate-500 font-bold uppercase tracking-wider md:tracking-widest">Keyboard Shortcuts & Efficiency</p>
          </div>
        </div>
      ...
      <div className="flex-1 flex flex-col md:flex-row overflow-hidden">
        {/* Sidebar Filters */}
        <aside className="max-md:shrink-0 flex flex-wrap gap-2 md:block w-full md:w-64 border-b md:border-b-0 md:border-r border-white/5 px-4 py-3 md:p-6 md:space-y-2">
          {['all', 'general', 'editor', 'navigation'].map(f => (
            <button
              key={f}
              onClick={() => setFilter(f)}
              className={`md:w-full text-center md:text-left px-3.5 py-2.5 md:px-4 md:py-3 max-md:min-h-[40px] rounded-xl text-[10px] md:text-xs font-black uppercase tracking-wider md:tracking-widest transition-all ${
        ...
        {/* Shortcuts Grid */}
        <main className="flex-1 overflow-y-auto p-4 md:p-10 space-y-4 md:space-y-8 no-scrollbar">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4">
            {filteredShortcuts.map((s, idx) => (
              <div key={idx} className={`p-4 md:p-6 max-md:gap-3 rounded-2xl border border-white/5 bg-white/2 hover:bg-white/5 transition-all flex items-center justify-between group ${lc}`}>
                <div className="flex items-center gap-3 md:gap-4 min-w-0">
                  <div className="w-10 h-10 shrink-0 rounded-xl bg-white/5 flex items-center justify-center text-slate-500 group-hover:text-indigo-500 transition-colors">
                    <s.icon size={18} />
                  </div>
                  <div className="min-w-0">
                ...
                <div className="flex items-center gap-1 md:gap-1.5 shrink-0">
                  {s.key.split(' + ').map((k, kIdx) => (
                    <React.Fragment key={kIdx}>
                      <kbd className="px-2.5 md:px-3 py-1.5 rounded-lg bg-[#0F172A] border border-white/10 text-[11px] font-black text-white shadow-lg min-w-[32px] text-center">{k}</kbd>
          ...
          <div className={`p-5 md:p-8 rounded-2xl md:rounded-[2rem] border border-dashed border-white/10 text-center space-y-2 ${theme === 'dark' ? 'bg-white/2' : 'bg-slate-50'}`}>
