...
const SettingsView = ({ theme, onClose }) => {
  ...
  return (
    ...
    >
      {/* Settings Sidebar */}
      <aside className={`w-full md:w-64 max-md:shrink-0 border-b md:border-b-0 md:border-r px-4 pt-1.5 pb-2.5 md:p-8 flex flex-col gap-1 transition-colors ${theme === 'dark' ? 'border-white/5 bg-white/2' : 'border-slate-100 bg-slate-50/50'}`}>
        <button 
          onClick={onClose}
          className="flex items-center gap-2 max-md:min-h-[40px] self-start md:self-auto mb-1 md:mb-8 text-slate-500 hover:text-white transition-colors group"
        >
          <ArrowLeft size={16} className="group-hover:-translate-x-1 transition-transform" />
          <span className="text-[11px] md:text-xs md:leading-4 font-black uppercase tracking-widest">Back to workspace</span>
        </button>
        ...
        {menuItems.map(item => (
          <button 
            key={item.name}
            className={`max-md:shrink-0 max-md:whitespace-nowrap max-md:min-h-[40px] text-left px-3.5 md:px-3 py-1.5 md:py-2 rounded-full md:rounded-lg text-[13px] font-medium transition-all ${
              item.active 
      ...
      {/* Settings Content */}
      <main className="flex-none md:flex-1 max-md:min-w-0 overflow-visible md:overflow-y-auto p-4 pt-4 md:p-12 max-w-4xl">
        <h1 className="text-xl leading-tight md:text-4xl md:leading-10 font-black mb-4 md:mb-16 tracking-tight">Team settings</h1>

        {/* General Section */}
        <section className="mb-6 md:mb-20">
          <h2 className="text-base md:text-2xl font-black mb-3 md:mb-8">General</h2>
          <div className="space-y-1">
            <label className="text-[10px] font-black uppercase tracking-widest text-slate-500">Name</label>
            <div className="text-base md:text-lg md:leading-7 font-bold tracking-wide">O.1</div>
            <div className="h-px w-full bg-white/5 mt-3 md:mt-4"></div>
          </div>
        </section>

        {/* Members Section */}
        <section className="mb-6 md:mb-20">
          <div className="flex flex-wrap md:flex-nowrap items-center justify-between gap-x-3 md:gap-x-0 mb-3 md:mb-8">
            <h2 className="text-base md:text-2xl font-black">Members (1)</h2>
          ...
          <div className="bg-[#D4AF37]/10 border border-[#D4AF37]/20 rounded-xl p-3.5 md:p-6 mb-2 md:mb-8 relative overflow-hidden group">
          ...
          {/* User List */}
          <div className="flex items-center justify-between gap-3 md:gap-0 py-3 md:py-4 group">
        ...
        {/* Groups Section */}
        <section>
          <h2 className="text-base md:text-2xl font-black mb-3 md:mb-8">Groups</h2>
          <div className={`bg-white/5 border border-white/5 rounded-xl p-6 md:p-12 flex flex-col items-center justify-center text-center ${theme === 'dark' ? '' : 'max-md:bg-slate-50 max-md:border-slate-200 max-md:rounded-2xl'}`}>
            <Users size={32} className="text-slate-700 mb-3 md:mb-4 max-md:w-7 max-md:h-7" />
